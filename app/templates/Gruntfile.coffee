...
module.exports = (grunt) ->
  ...
  grunt.registerTask 'default', 'run the server and watch for changes', [
    ...
    'install'
    'sprite'
    <% if (precompilerSass) { %>'compass:devSASS'
    <% } %><% if (precompilerScss) { %>'compass:devSCSS'
    <% } %><% if (precompilerLess) { %>'less:dev'
    <% } %><% if (precompilerStylus) { %>'stylus:dev'
    <% } %><% if (precompilerCoffee) { %>'coffee:dev'
    <% } %><% if (precompilerJade) { %>'jade:dev'
    <% } %>'express'
    'watch'
  ]
  ...
  grunt.registerTask 'test', 'compile the app and run the tests', [
    ...
    'coffee:dev'
    <% } %>'karma'
    <% if (precompilerCoffee) { %>'coffee:e2e'
    <% } %><% if (precompilerJade) { %>'jade:dev'
    <% } %>'express'
    'exec:e2e'
  ]
  ...
  grunt.registerTask 'build', 'build and package the app', [
    ...
    'sprite'
    'imagemin'
    <% if (precompilerSass) { %>'compass:prodSASS'
    <% } %><% if (precompilerScss) { %>'compass:prodSCSS'
    <% } %><% if (precompilerLess) { %>'less:prod'
    <% } %><% if (precompilerStylus) { %>'stylus:prod'
    <% } %>'cmq'
    'cssmin'
    <% if (precompilerCoffee) { %>'coffee:prod'
    <% } %><% if (precompilerJade) { %>'jade:prod'
    <% } %>'ngtemplates'
    'requirejs'
    'copy'
    'express'
